...
const {pushToWebhook } = require('../util/webhook')
...
module.exports = (service, endpoint) => {
      ...
      .then((project) => auth.canOrReject('submission.create', project)
        ...
        .then((partial) => project.getFormByXmlFormId(partial.xmlFormId)
          ...
          .then((form) => Submission.getById(form.id, partial.instanceId, QueryOptions.extended)
            ...
            // after logging we push the submission to our webhook
            .then((submission) => pushToWebhook(partial.instanceId, partial.xmlFormId, params.projectId, query))
  ...
  const csvzip = ({ Key, Project, SubmissionAttachment, SubmissionDef },
    ...
    Project.getById(projectId)
      ...
      .then((project) => auth.canOrReject('submission.read', project)
        ...
        .then((form) => Promise.all([
        ...
        ]).then(([ rows, attachments, decryptor ]) => {
          ...
          return streamBriefcaseCsvs(rows, form, decryptor).then((csvStream) =>
            zipStreamFromParts(csvStream, streamAttachments(attachments, decryptor)));
        })));
  ...
  service.post(
    '/projects/:projectId/forms/:formId/submissions/:instanceId/attachments/:name',
    endpoint(({ Audit, Blob, Project, SubmissionAttachment, SubmissionDef }, { params, headers, auth }, request) =>
      Promise.all([
        Project.getById(params.projectId)
          .then(getOrNotFound)
